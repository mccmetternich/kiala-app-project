...
export default function DualOfferComparison({
  ...
  return (
    <div className="my-8">
      {/* Exclusive Banner */}
      {showExclusiveBanner && (
        <div className="bg-gradient-to-r from-accent-800 to-accent-500 text-white rounded-t-2xl px-4 py-3 text-center">
      ...
      <div className={`bg-gradient-to-br from-gray-50 to-white p-6 md:p-8 ${showExclusiveBanner ? 'rounded-b-2xl' : 'rounded-2xl'} shadow-xl border border-gray-100`}>
        ...
        <div className="grid md:grid-cols-2 gap-6 relative">
          ...
          <div className="hidden md:flex absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
            <div className="w-16 h-16 bg-gradient-to-br from-accent-800 to-accent-900 text-white rounded-full flex items-center justify-center font-bold text-lg shadow-xl border-4 border-white">
              {vsText}
            </div>
          </div>
...
function OfferCard({ offer, side, widgetId }: { offer: Offer; side: 'left' | 'right'; widgetId?: string }) {
  ...
  return (
    <div className={`relative ${side === 'left' ? 'md:pr-4' : 'md:pl-4'}`}>
      ...
      {isHighlighted && (
        <div className="absolute -inset-1 bg-gradient-to-r from-accent-600 to-accent-500 rounded-2xl opacity-30 blur-lg"></div>
      )}
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <div className="text-center mb-5">
            {offer.image && (
              <div className="relative inline-block mb-4">
                ...
                />
                {isHighlighted && (
                  <div className="absolute -top-2 -right-2 w-10 h-10 bg-gradient-to-br from-accent-800 to-accent-500 rounded-full flex items-center justify-center shadow-lg">
          ...
          <TrackedLink
            ...
            className={`flex items-center justify-center gap-2 w-full py-4 rounded-xl font-bold text-lg transition-all text-center ${
              isHighlighted
                ? 'bg-gradient-to-r from-accent-800 to-accent-500 hover:from-accent-900 hover:to-accent-600 text-white shadow-lg hover:shadow-xl transform hover:-translate-y-1'
