...
export default function SiteHomePage() {
      ...
      try {
        ...
        if (siteResponse.ok) {
          ...
          if (site) {
            ...
            // Fetch homepage (slug: 'index')
            const pageResponse = await fetch(`/api/pages?siteId=${site.id}`);
            if (pageResponse.ok) {
              const pagesData = await pageResponse.json();
              const homepage = pagesData.find((p: any) => p.slug === 'index');
              if (homepage && homepage.published) {
                setPage(homepage);
              } else {
                // No homepage found, redirect to articles
                window.location.href = `/site/${siteId}/articles`;
                return;
              }
            }
          } else {
  ...
  // No homepage found, redirect to articles (this handles the redirect after loading)
  if (!page) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
